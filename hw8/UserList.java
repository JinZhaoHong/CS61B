...
public class UserList {
    ...
    **/
    public static void quickSort(String sortFeature, CatenableQueue<User> q){ 
        ...
        //int qSize = q.size();
        //int pivot = (int) (Math.random() * qSize);
        ...
        if (sortFeature.equals("id")) {
            //pivot = q.nth(pivot).getId();
            int pivot = q.front().getId();
        ...
        if (sortFeature.equals("pages")) {
            //pivot = q.nth(pivot).getPagesPrinted();
            int pivot = q.front().getPagesPrinted();
            partition(sortFeature, q, pivot, qLess, qEqual, qGreater);
            quickSort(sortFeature, qLess);
            quickSort(sortFeature, qGreater);
            qGreater.append(userQueue);
            q.append(qLess);
            q.append(qEqual);
            q.append(qGreater);
        }
    ...
    **/
    public CatenableQueue<CatenableQueue<User>> makeQueueOfQueues(){
        //Replace with solution.
        CatenableQueue<CatenableQueue<User>> returnQueueOfQueues = new CatenableQueue<CatenableQueue<User>>();
        while (!userQueue.isEmpty()) {
            CatenableQueue<User> oneUser = new CatenableQueue<User>();
            User u = userQueue.dequeue();
            oneUser.enqueue(u);
            returnQueueOfQueues.enqueue(oneUser);
        }
        return returnQueueOfQueues;
    }
    ...
    **/
    public static CatenableQueue<User> mergeTwoQueues(String sortFeature, CatenableQueue<User> q1, CatenableQueue<User> q2){
        ...
        while (!q1.isEmpty() && !q2.isEmpty()) {
            int cmp = 0;
            if (sortFeature.equals("id")) {
                cmp = q1.front().compareById(q2.front());
            }
            if (sortFeature.equals("pages")) {
                cmp = q1.front().compareByPagesPrinted(q2.front());
            }
            if (cmp < 0) {
                User u = q1.dequeue();
                sorted.enqueue(u);
    ...
    **/
    public void mergeSort(String sortFeature){
        ...
        while (queueOfQueue.size() != 1) {
            CatenableQueue<User> q1 = queueOfQueue.dequeue();
            CatenableQueue<User> q2 = queueOfQueue.dequeue();
            CatenableQueue<User> sorted = mergeTwoQueues(sortFeature, q1, q2);
            queueOfQueue.enqueue(sorted);      
        }
    ...
    **/
    public void sortByBothFeatures(){
        ...
        //quickSort("pages");
        //quickSort("id");
        // or we can use mergesort instead
        mergeSort("pages");
        mergeSort("id");
    }
